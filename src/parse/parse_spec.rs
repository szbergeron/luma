...
use crate::{lex::TokenWrapper, parse::*};
...
impl<'lexer> Parser<'lexer> {
    pub fn parse_spec(&mut self, t: &TokenProvider, current_path: &Path) -> ParseResult<Spec> {
        ...
        }) = t.lh.la(0)
        {
            ...
            res.push(
                self.parse_spec_declaration(&t, current_path)
                    .catch(&mut t)
                    .join_hard(&mut t)?,
            );
            println!("Got a spec");
    ...
    pub fn parse_spec_declaration(
        &mut self,
        t: &TokenProvider,
        current_path: &Path,
    ) -> ParseResult<(Vec<IStr>, FileRole)> {
        ...
        let frd = match dec_type.slice.resolve() {
            ...
            _other => t
                .failure(ParseResultError::SemanticIssue(
                    "expected source, spec, or data, found alternative value",
                    dec_type.start,
                    dec_type.end,
                ))
                .catch(&mut t)
        ...
        let file_base = current_path
            .parent()
            .expect("we weren't in a directory somehow?");
        ...
        let inner = match frd {
            FileRoleDescriminant::Data => FileRole::Data(DataFile {
                location: file_path,
            }),
            FileRoleDescriminant::Spec => FileRole::Spec(SpecFile {
                location: file_path,
            }),
            FileRoleDescriminant::Source => FileRole::Source(SourceFile {
                location: file_path,
            }),
        };
